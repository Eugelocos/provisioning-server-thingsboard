...
class CustomerService {
    async getOrCreateWithIncrement(customerName: string): Promise<any> {
        ...
        let customer = cache.get(baseCacheKey) as Record<string, any> | undefined;
        ...
        try {
            ...
            customer = await createResponse.json() as Record<string, any>;
    ...
    private async findAvailableCustomerName(
        baseName: string,
        existingCustomers: Array<{ title: string }>
    ): Promise<string> {
...
class DeviceService {
    async createOrGet(
        ...
        if (cached) {
            const cachedDevice = cached as { device?: { id?: { id?: string } } };
        ...
        try {
            ...
            const existingBySerial = searchData.data.find(
                (d) => d.name === deviceName || d.label === serialNumber
            );
    ...
    private async findDeviceByIdentity(
        ...
        try {
            ...
            const searchData = await searchResponse.json() as {
                data: Array<{ additionalInfo?: { description?: string } }>;
            };

            return (
                searchData.data.find(
                    (d) => d.additionalInfo?.description === deviceIdentity
                ) || null
            );
        } catch (error) {
    ...
    async cloneAndAssign(
        ...
        try {
            ...
            if (!template) {
                ...
                template = await templateResp.json() as {
                    configuration: any;
                    id?: { id: string };
                    title?: string;
                    [key: string]: any;
