...
// ================================
// IMPORTACIÓN DE MÓDULOS V2
// ================================
// Importar las instancias desde backend-extensions
const backend_extensions_1 = require("./backend-extensions");
// Importar las funciones de registro de rutas
const api_routes_v2_1 = require("./api-routes-v2");
...
class HighPerformanceCache {
    ...
    set(key, data, ttl = PERFORMANCE_CONFIG.cacheTimeout) {
        this.cache.set(key, {
            data,
            timestamp: Date.now(),
            ttl,
...
class IPRateLimiter {
    ...
    maxRequests = PERFORMANCE_CONFIG.rateLimitMax;
    canProceed(ip) {
        const now = Date.now();
...
class GlobalRateLimiter {
    requests = [];
    windowMs = PERFORMANCE_CONFIG.rateLimitWindow;
    maxRequests = PERFORMANCE_CONFIG.globalRateLimit;
    canProceed() {
        const now = Date.now();
        // Limpiar requests antiguos
...
class ConcurrencyController {
    activeRequests = 0;
    queue = [];
    maxConcurrent = PERFORMANCE_CONFIG.maxConcurrentRequests;
...
class OptimizedHTTPClient {
    tokenCache = null;
    // Tipos para las respuestas de ThingsBoard
    async makeRequest(url, options, retries = TB_CONFIG.maxRetries) {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), TB_CONFIG.requestTimeout);
        try {
            ...
            if (!response.ok && retries > 0 && response.status >= 500) {
                ...
                await new Promise((resolve) => setTimeout(resolve, TB_CONFIG.retryDelay));
...
app.use("/api/*", async (c, next) => {
    const ip = process.env.CF_CONNECTING_IP ||
...
app.use("/api/v1/*", async (c, next) => {
    ...
    if (!expectedKey) {
        console.warn("⚠️ PROVISIONING_API_KEY no configurada — se permite acceso libre");
...
class CustomerService {
    async getOrCreateWithIncrement(customerName) {
        const baseCacheKey = `customer:${customerName}`;
        let customer = cache.get(baseCacheKey);
        if (customer) {
            return customer;
        }
        try {
            // Buscar clientes existentes con el nombre base
            const searchResponse = await httpClient.request(`/api/customers?pageSize=100&page=0&textSearch=${encodeURIComponent(customerName)}`);
            if (!searchResponse.ok) {
                throw new Error(`Error searching customer: ${searchResponse.statusText}`);
            }
            const searchData = await searchResponse.json();
            // Buscar coincidencia exacta primero
            const exactMatch = searchData.data.find((c) => c.title === customerName);
            if (exactMatch) {
                cache.set(baseCacheKey, exactMatch);
                return exactMatch;
            }
            // Si no hay coincidencia exacta, buscar el siguiente número disponible
            const finalCustomerName = await this.findAvailableCustomerName(customerName, searchData.data);
            ...
            if (!createResponse.ok) {
                if (createResponse.status === 480) {
                    console.warn(`Customer ${finalCustomerName} posiblemente ya existe. Intentando recuperar...`);
                    // Buscar otra vez con el nombre final
                    const retryResponse = await httpClient.request(`/api/customers?pageSize=100&page=0&textSearch=${encodeURIComponent(finalCustomerName)}`);
                    const retryData = await retryResponse.json();
                    const existing = retryData.data.find((c) => c.title === finalCustomerName);
                    if (existing)
                        return existing;
                }
            ...
            console.log(`Customer created: ${finalCustomerName} (${customer.id.id})`);
            return customer;
        }
        catch (error) {
    ...
    async findAvailableCustomerName(baseName, existingCustomers) {
        const pattern = new RegExp(`^${baseName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?: (\\d{2}))?$`);
    ...
    async getOrCreate(customerName) {
...
class UserService {
    httpClient;
    emailService;
    constructor(httpClient, emailService) {
        this.httpClient = httpClient;
        this.emailService = emailService;
    }
    async createOrGetCustomerUser(params) {
        const { email, customerId, customerName, deviceName = 'Dispositivo', firstName, lastName, sendEmail = true, } = params;
        try {
    ...
    async findUserByEmail(email) {
        try {
            const searchResponse = await this.httpClient.request(`/api/users?pageSize=10&page=0&textSearch=${encodeURIComponent(email)}`);
            if (!searchResponse.ok)
                return null;
            const searchData = await searchResponse.json();
            return searchData.data.find((u) => u.email === email) || null;
        }
        catch (error) {
            return null;
        }
    ...
    async activateUser(userId, password) {
        try {
            ...
            const activationResponse = await this.httpClient.request(`/api/user/${userId}/activationLink`, { method: 'GET' });
...
class DeviceService {
    async createOrGet(deviceName, serialNumber, customer, sensorName) {
        const cacheKey = `device:${serialNumber}`;
        const cached = cache.get(cacheKey);
        if (cached) {
            const cachedDevice = cached;
            const deviceId = cachedDevice?.device?.id?.id ?? "";
            const stillExists = await this.verifyDeviceExists(deviceId);
            if (stillExists) {
        ...
        try {
            ...
            const existingDevice = await this.findDeviceByIdentity(deviceIdentity, customer.id.id);
            if (existingDevice) {
                console.log(`🔄 Recambio detectado: ${existingDevice.name} → ${deviceName}`);
                const updatedDevice = await this.updateDeviceForReplacement(existingDevice, deviceName, serialNumber);
                const credentials = await this.getDeviceCredentials(updatedDevice.id.id);
                const result = {
            ...
            const searchResponse = await httpClient.request(`/api/tenant/devices?pageSize=100&page=0&textSearch=${encodeURIComponent(serialNumber)}`);
            if (!searchResponse.ok) {
                throw new Error(`Error searching device: ${searchResponse.statusText}`);
            }
            const searchData = await searchResponse.json();
            const existingBySerial = searchData.data.find((d) => d.name === deviceName || d.label === serialNumber);
            if (existingBySerial) {
                // Obtener credenciales del existente
                const credentialsResponse = await httpClient.request(`/api/device/${existingBySerial.id.id}/credentials`);
            ...
            try {
                await httpClient.request(`/api/customer/${customer.id.id}/device/${device.id.id}`, {
                    method: "POST",
                    body: "{}",
                });
            }
            ...
            const credentialsResponse = await httpClient.request(`/api/device/${device.id.id}/credentials`);
            const credentials = credentialsResponse.ok
                ? await credentialsResponse.json()
                : null;
            const result = {
    ...
    async findDeviceByIdentity(deviceIdentity, customerId) {
        try {
            const searchResponse = await httpClient.request(`/api/customer/${customerId}/devices?pageSize=1000&page=0`);
            if (!searchResponse.ok)
                return null;
            const searchData = await searchResponse.json();
            return (searchData.data.find((d) => d.additionalInfo?.description === deviceIdentity) || null);
        }
        catch (error) {
    ...
    async updateDeviceForReplacement(existingDevice, newDeviceName, newSerialNumber) {
    ...
    async getDeviceCredentials(deviceId) {
        try {
            const credentialsResponse = await httpClient.request(`/api/device/${deviceId}/credentials`);
            if (!credentialsResponse.ok) {
                throw new Error(`Error getting device credentials: ${credentialsResponse.statusText}`);
            }
...
class DashboardService {
    async cloneAndAssign(deviceId, deviceName, customerId) {
        if (!TB_CONFIG.dashboardTemplateId) {
            console.warn("⚠️ No dashboard template ID configured, skipping dashboard creation");
            return null;
        }
        ...
        try {
            ...
            if (!template) {
                const templateResp = await httpClient.request(`/api/dashboard/${TB_CONFIG.dashboardTemplateId}`);
                if (!templateResp.ok) {
            ...
            const clonedConfig = this.replaceDeviceInConfig(template.configuration, deviceId);
            ...
            // Asignar a customer
            try {
                await httpClient.request(`/api/customer/${customerId}/dashboard/${dashboard.id.id}`, {
                    method: "POST",
                });
            }
            catch (assignError) {
...
// UTILIDADES
// ================================
function determineCustomerName(serialNumber) {
...
function validateProvisioningRequest(body) {
    if (!body.serialNumber)
        return "serialNumber es requerido";
    if (typeof body.serialNumber !== "string")
        return "serialNumber debe ser string";
    if (body.serialNumber.length < 3)
        return "serialNumber muy corto";
    if (body.serialNumber.length > 50)
        return "serialNumber muy largo";
    if (!/^[a-zA-Z0-9_-]+$/.test(body.serialNumber))
        return "serialNumber contiene caracteres inválidos";
    if (!body.sensorName)
        return "sensorName es requerido";
    if (typeof body.sensorName !== "string")
        return "sensorName debe ser string";
    if (body.sensorName.length < 2)
        return "sensorName muy corto";
    if (body.sensorName.length > 50)
        return "sensorName muy largo";
    if (body.customerName) {
        if (typeof body.customerName !== "string")
            return "customerName debe ser string";
        if (body.customerName.length < 2)
            return "customerName muy corto";
        if (body.customerName.length > 100)
            return "customerName muy largo";
    }
    // NUEVA VALIDACIÓN: userEmail (opcional)
    if (body.userEmail) {
        if (typeof body.userEmail !== "string")
            return "userEmail debe ser string";
...
// ================================
// INICIALIZACIÓN DE MÓDULOS V2
// ================================
// Instancias de servicios ya están importadas
console.log('🔧 Inicializando sistemas v2...');
// Inicializar los managers
(async () => {
    await backend_extensions_1.productManager.initialize();
    await backend_extensions_1.templateEngine.initialize();
    console.log('✅ Sistemas v2 inicializados');
})();
// ================================
// ENDPOINTS
// ================================
    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <p><strong>Capacidad:</strong> 3200+ requests/hour | <strong>Uptime:</strong> ${Math.round(stats.uptime / 60)} min</p>
            </div>
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    ...
                    <div class="stat-detail">${stats.cache.hits} hits, ${stats.cache.misses} misses</div>
                </div>
                <div class="stat-card">
                    <div class="stat-title">Rate Limiting</div>
                    <div class="stat-value">${stats.rateLimiting.remaining}</div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-detail">Active: ${stats.concurrency.active} | Queued: ${stats.concurrency.queued}</div>
                </div>
                <div class="stat-card">
            ...
            <div class="config">
                ...
                <div class="code">
#define PROVISIONING_SERVER "${c.req.header("host") || "your-app.railway.app"}"<br>
...
app.post("/api/v1/devices/provision", async (c) => {
    ...
    try {
        ...
        const { serialNumber, productName = "TempLogger", customerName, sensorName, userEmail, // ← NUEVO PARÁMETRO
        ...
        // Crear dispositivo
        const deviceResult = await deviceService.createOrGet(deviceName, serialNumber, customer, sensorName);
        ...
        if (TB_CONFIG.dashboardTemplateId) {
            try {
                dashboard = await dashboardService.cloneAndAssign(deviceResult.device.id.id, deviceName, customer.id.id);
            }
            catch (dashError) {
        ...
        backend_extensions_1.analyticsEngine.trackEvent({
            ...
            data: {
                error: error instanceof Error ? error.message : "Unknown error",
                serialNumber: body?.serialNumber || "unknown",
                processingTime: processingTime,
            },
        });
        return c.json({
...
app.get("/api/v1/customers", async (c) => {
    try {
        ...
        const response = await httpClient.request("/api/customers?pageSize=100&page=0");
        if (!response.ok) {
...
app.get("/api/v1/devices/:serialNumber", async (c) => {
    try {
        ...
        const searchData = await searchResponse.json();
        const device = searchData.data.find((d) => d.label === serialNumber);
        if (!device) {
...
app.post("/api/v1/devices/provision/batch", async (c) => {
    try {
        ...
        if (!Array.isArray(devices) || devices.length === 0) {
            return c.json({ success: false, message: "devices array is required" }, 400);
        }
        if (devices.length > 50) {
            return c.json({ success: false, message: "Maximum 50 devices per batch" }, 400);
        }
        const startTime = Date.now();
        const results = [];
        const errors = [];
        ...
        for (let i = 0; i < devices.length; i += chunkSize) {
            ...
            const chunkResults = await Promise.allSettled(chunk.map(async (deviceData) => {
                const { serialNumber, productName = "TempLogger" } = deviceData;
                const deviceName = `${productName}_${serialNumber}`;
                const customerName = customerOverride || determineCustomerName(serialNumber);
                const customer = await customerService.getOrCreate(customerName);
                const deviceResult = await deviceService.createOrGet(deviceName, serialNumber, customer);
                return {
                    serialNumber,
                    device: {
                        id: deviceResult.device.id.id,
        ...
        return c.json({
            success: false,
            message: error instanceof Error ? error.message : "Batch provisioning failed",
        }, 500);
    }
});
// ================================
// REGISTRAR RUTAS V2
// ================================
console.log('🔧 Registrando rutas v2...');
(0, api_routes_v2_1.registerProductRoutes)(app);
(0, api_routes_v2_1.registerTemplateRoutes)(app);
(0, api_routes_v2_1.registerAnalyticsRoutes)(app);
(0, api_routes_v2_1.registerSystemRoutes)(app);
console.log('✅ Rutas v2 registradas');
// ================================
...
httpClient
    ...
    .catch(() => console.log("⚠️  Initial ThingsBoard connection failed - will retry on demand"));
