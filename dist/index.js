...
class IPRateLimiter {
    constructor() {
        ...
        this.windowMs = PERFORMANCE_CONFIG.rateLimitWindow;
        this.maxRequests = PERFORMANCE_CONFIG.rateLimitMax;
    }
    canProceed(ip) {
        const now = Date.now();
...
class GlobalRateLimiter {
    constructor() {
        this.requests = [];
        this.windowMs = PERFORMANCE_CONFIG.rateLimitWindow;
        this.maxRequests = PERFORMANCE_CONFIG.globalRateLimit;
    }
    canProceed() {
        const now = Date.now();
        // Limpiar requests antiguos
...
class ConcurrencyController {
    constructor() {
        ...
        this.queue = [];
        this.maxConcurrent = PERFORMANCE_CONFIG.maxConcurrentRequests;
    }
...
class DeviceService {
    async createOrGet(deviceName, serialNumber, customer, sensorName) {
        ...
        if (cached) {
            ...
            const stillExists = await this.verifyDeviceExists(cached?.device?.id?.id ?? "");
            if (stillExists) {
